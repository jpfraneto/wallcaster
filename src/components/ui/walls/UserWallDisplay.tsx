...
interface UserWallDisplayProps {
  chosenWall: {
    castHash: string;
    username: string;
    pfp: string;
...
export default function UserWallDisplay({
  chosenWall,
  onClose,
}: UserWallDisplayProps) {
  ...
  const fetchReplies = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/wallcaster/wall/${
          chosenWall.castHash
        }`,
        {
  ...
  }, [text, submitting, chosenWall.castHash, fetchReplies]);
    ...
    >
      ...
      >
        ...
        <div className="p-4 border-b border-gray-700 flex items-center gap-3">
          <img
            src={chosenWall.pfp}
            alt={chosenWall.username}
            className="w-12 h-12 rounded-full"
            loading="lazy"
          />
          <div className="flex-1">
            <h3 className="font-bold text-lg">@{chosenWall.username}</h3>
        ...
        <div className="p-4 border-b border-gray-700">
          <div className="flex gap-2">
            <textarea
              ...
              placeholder={`Write on @${chosenWall.username}'s frick...`}
