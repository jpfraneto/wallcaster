...
__export(src_exports, {
  ...
  generateNonce: () => import_siwe4.generateSiweNonce,
  get: () => get,
...
var import_siwe4 = require("viem/siwe");
...
var import_siwe2 = require("viem/siwe");
    ...
    () => {
      const siweMessage = (() => {
        if (typeof params === "string") {
          return (0, import_siwe.parseSiweMessage)(params);
        }
        return params;
      })();
      const isValid = (0, import_siwe.validateSiweMessage)({ message: siweMessage });
      if (isValid) {
        return siweMessage;
      }
...
var defaultPublicClient = (0, import_viem.createPublicClient)({
...
var verify = async (nonce, domain, message, signature, options = {
  ...
  if (auth.isErr())
    return (0, import_neverthrow3.err)(auth.error);
  return (0, import_neverthrow3.ok)(auth.value);
};
...
var verifySiweMessage = async (message, signature, client) => {
  try {
    const verified = await (0, import_siwe2.verifySiweMessage)(client, { message, signature });
    if (verified) {
      return (0, import_neverthrow3.ok)(void 0);
    }
    return (0, import_neverthrow3.err)(new AuthClientError("unauthorized", "Failed to verify message"));
  } catch (e) {
    return (0, import_neverthrow3.err)(new AuthClientError("unknown", e));
  }
};
var verifyAuthorizedSigner = async (response, options) => {
  ...
  if (acceptAuthAddress) {
    const authAddressResult = await import_neverthrow3.ResultAsync.fromPromise(
      isValidAuthAddress(signer, BigInt(response.fid)),
      (e) => new AuthClientError("unavailable", e)
    );
    if (authAddressResult.isErr()) {
      return (0, import_neverthrow3.err)(authAddressResult.error);
    }
    if (authAddressResult.isOk() && authAddressResult.value === true) {
      return (0, import_neverthrow3.ok)({ ...response, authMethod: "authAddress" });
    }
    const fidResult = await import_neverthrow3.ResultAsync.fromPromise(
      getFid(signer),
      (e) => new AuthClientError("unavailable", e)
    );
    if (fidResult.isErr()) {
      return (0, import_neverthrow3.err)(fidResult.error);
    }
    ...
    return (0, import_neverthrow3.ok)({ ...response, authMethod: "custody" });
  }
  return import_neverthrow3.ResultAsync.fromPromise(getFid(signer), (e) => {
...
var mergeResources = (message) => {
  return parseResources(message).andThen((resources) => {
    return (0, import_neverthrow3.ok)({ ...resources, data: message, success: true });
  });
};

// src/actions/app/verifySignInMessage.ts
var verifySignInMessage = async (client, { nonce, domain, message, signature, acceptAuthAddress }) => {
...
var import_siwe3 = require("viem/siwe");
var build = (params) => {
  ...
  return (0, import_neverthrow4.ok)({ siweMessage, message: (0, import_siwe3.createSiweMessage)(siweMessage) });
};
...
var viemConnector = (args) => {
  const transport = (() => {
    if (args?.rpcUrls) {
      return (0, import_viem2.fallback)(args.rpcUrls.map((rpcUrl) => (0, import_viem2.http)(rpcUrl)));
    }
    if (!args?.rpcUrl) {
      console.warn("No `rpcUrl` provided to Viem connector, using public endpoint. Do not use this in production");
    }
    return (0, import_viem2.http)(args?.rpcUrl);
  })();
  ...
  const isValidAuthAddress = async (authAddress, fid) => {
    const paddedAddress = (0, import_viem2.encodeAbiParameters)([{ name: "authAddress", type: "address" }], [authAddress]);
